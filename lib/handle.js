...
function handleRequest (req, res, postData, cwd) {
    let getData = url.parse(req.url),
        data = getData.query,
        args = {};
    if (postData) {
        data = postData;
    ...
    if (!Object.keys(args).length)
        args = qs.parse(data);
    res = STD(res);
    let api = require(cwd('api')),
        cachesRulesfile = cwd('caches.txt'),
        funcName = getData.pathname.replace(/\//g, ''),
        fmt = 'json';
    if (args) {
    ...
    switch (funcName) {
    case 'favicon.ico':
        return res.stderr('Not found', fmt);
    case 'caches.txt':
        let out = 'The caches.txt does not exist';
        if (fs.existsSync(cachesRulesfile))
            out = fs.readFileSync(cachesRulesfile, 'utf8')
        return res.stdout(out);
    case '':
        return res.stderr('You did not specify a function name', fmt);
    }
    ...
    if (typeof(func) == 'function') {
        let caches = new Caches(cachesRulesfile);
        let cacheTbl = func + '_' + fmt;
        let cachedOut = caches.resolve(cacheTbl, data);
        console.log('Function:', funcName, '[', data, ']')
        if (cachedOut) {
            ...
            let context = {
                'return': function (result) {
                    ...
                    out = serialize(result, fmt);
                    console.log('Format:', fmt)
                    caches.save(cacheTbl, data, out);
                    res.writeHead(200, {'Content-Type': FORMATS[fmt]});
                    res.end(out);
                }
            }
