...
function handleRequest (request, response, api, caches) {
    ...
    if (typeof(func) == 'function') {
        ...
        console.log('Function "' + funcName + '" with [', args, ']');
        if (cachedOut) {
            ...
            if (out !== undefined) {
                ...
                if (func.toString().indexOf('.return') == -1) {
                    // context.return('');
                    logger.warn('The function "' + funcName + '" should return something, or delay it via self.return');
                    return response.stderr('The function should return something, or delay it via self.return ' +
                                           '(see https://smappi.org/documentation/the-deferred-return/)', fmt);
                }
            }
        }
    } else {
