...
const getTests = (content) => {
    ...
    const comments = functionComments.map( comment => {
        const exampleNodes = comment.tags.filter(type => type === 'example');
        const examples = [].concat(exampleNodes.map(exampleNode =>
...
const contentsToTapSpec = (rootDir, filename, content, withExtra) => {
  ...
  tap.test('${mn}', (t) => {
      t.plan(${examplesCnt});
      ${keep(comments).map((comment) => `${commentToTapSpec(comment).join('\n\n')}`).join('\n\n// -------------------------------------------------')}
  }, {timeout: 60000});
  ${structure}
  `;
}
