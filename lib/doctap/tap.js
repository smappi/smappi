...
const getExampleCode = ({ testCase, expectedResult }, funcName) => {
    ...
    let code = ts[ts.length - 1].replace(`${funcName}(`, `${funcName}.call({'return': testSame}, `);
...
const commentToTapSpec = (comment) => {
    ...
    (function () {
        ...
        ${com.joined}
        if (${ctx.name}.toString().indexOf('.return') < 0 || result !== undefined) {
            testSame(result);
        }
    })();`
    }, comment.examples);
...
function adaptResultByMask (foundResult, wantedCode) {
    ...
    let buildCompareResult = (found, wanted) => {
        // TODO OPTIMIZATION: compare only when found and wanted is one type
        // TODO OPTIMIZATION: and str + regexp, number + regexp, [] == [], {} == {}
        if (found === wanted) // When objects is equal, then return wanted
            return wanted;
        let result = found; // default is found
        if (result === undefined) { // undefined is object
            return result;
        }
        else if (wanted && wanted.constructor.name == 'RegExp') {
                ...
                for (; wIndex < wanted.length; wIndex++) {
                    ...
                    if (wValue === '...') {
                        ...
                        for (; fIndex < found.length; fIndex++) {
                            ...
                            // TODO: Make digest ForCompare Func
                            let fCompare = fValue, wCompare = wValue;
                            if (fValue && fValue.constructor.name == 'RegExp')
                                fCompare = fCompare.toString();
                            if (fValue && wValue.constructor.name == 'RegExp')
                                wCompare = wCompare.toString();
                            if (JSON.stringify(fCompare) == JSON.stringify(wCompare)) {
                                result.push(wValue);
                                break;
                            }
                        }
