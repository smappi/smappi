...
const getTests = (content) => {
    ...
    const functionComments = parsedContent.filter(c =>
                                    c.ctx &&
                                    (c.ctx.type === 'method' ||
                                     c.ctx.type === 'function' ||
                                     (c.ctx.type === 'declaration' && c.ctx.value.includes('=>'))))
    const comments = functionComments.map( comment => {
        const exampleNodes = comment.tags.filter(tag => tag.type == 'example');
        const examples = [].concat.apply([], exampleNodes.map(exampleNode => commentParser.run(exampleNode.string)));
...
const getModuleName = (rootDir, filename, parsedContent) => {
    const moduleBlock = parsedContent.find((block) =>
                                  block.tags && block.tags.find( type =>
                                                          type === 'example'))

    if (moduleBlock) {
        const moduleTag = moduleBlock.tags.find(type => type === 'module');
...
const commentToTapSpec = (comment) => {
    ...
    return comment.examples.map(function (example) {
...
const contentsToTapSpec = (rootDir, filename, content, withExtra) => {
    const tests = getTests(content),
	  comments = keep(tests.comments);
    const moduleName = getModuleName(rootDir, filename, tests.source);
    const mn = escapeString(moduleName);
    let examplesCnt = 0;
    if (comments && comments.length) {
        examplesCnt = comments.map((x) => x.examples.length).reduce((a, b) => a + b);
    }
    let structure = withExtra ? 'process.stdout.write(`STRUCTURE ${JSON.stringify(funcArgs)} \n\n`);' : '';
    return `
    ...
    tap.test('${mn}', (t) => {
	t.plan(${examplesCnt});
	${comments.map((comment) => `${commentToTapSpec(comment).join('\n\n')}`).join('\n\n// -------------------------------------------------')}
    }, {timeout: 60000});
    ${structure}
    `;
}
