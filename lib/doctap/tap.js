...
const getExampleCode = ({ testCase, expectedResult }) => {
    ...
    let code = ts[ts.length - 1];
    let func = code.match(/(.+?)\(/);
    if (func)
        func = func[1];
    code = code.replace('(', '.call({return: deferCheck}, ');
    ts[ts.length - 1] = `var result = ${code}, func = ${func};`;
...
const commentToTapSpec = (comment) => {
    ...
    return map(function (example) {
        ...
        return `
    (function () {
        let deferCheck = function (result) {
            t.same(
        ...
        ${com.joined}
        if (func.toString().indexOf('.return') < 0 || result !== undefined) deferCheck(result);
    })();`
    }, comment.examples);
}
...
function adaptResultByMask (foundResult, wantedCode) {
    ...
    let buildCompareResult = (found, wanted) => {
        ...
        if (wanted && wanted.constructor.name == 'RegExp') {
