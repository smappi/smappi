    ...
    if (typeof(params) == 'object') {
        for (let i = 0; paramName = Object.keys(params)[i]; i++) {
            ...
            if (param.__isArray) {
                ...
                isArray = params.__isArray;
                delete params.__isArray;
            }
            if (isArray && !param.__type) {
                globalArray = true;
            }
            if (param.__type) {
                let el = buildElement(paramName, param.__type, isArray, externalTypes);
                els.push(el.el);
                additionalTypes = additionalTypes.concat(el.additionalTypes);
            } else {
                let el = buildElement(paramName, param, isArray, externalTypes);
                if (isArray) {
                    globalArray = true;
                }
                els.push(el.el);
...
let buildElement = function(name, type, isArray, externalTypes){
    let elType = buildType(type, '', externalTypes, name),
        additionalTypes = [],
        el = '';
    if (!isArray && typeof(type) == 'object'){
        isArray = type.__isArray
    }
    if (elType.typeName) {
